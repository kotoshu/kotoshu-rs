// Semantic candidate generation — the merge half of the engine worker
// (plan 112 port of the playground semantic-merge module, kept verbatim
// in behavior).
//
// The dictionary sweep can only rank candidates that already look like
// the misspelling; the wasm engine semanticSuggest (@kotoshu/wasm
// 0.3.0+) asks the mini embedding tier for the nearest VOCABULARY words
// to the typed word — an out-of-vocabulary typo embeds through its
// character n-grams, so the intended word often comes back as top-1
// outright ("catt" -> cat at cosine 1.0). This module folds those
// generated neighbors into the dictionary candidates ahead of the
// context rerank, so ordering stays the rerank job.
//
// Kept pure (no worker, no wasm imports): the merge is exactly the kind
// of logic that deserves a unit check, and scripts/worker_node_smoke.mjs
// drives this file through the full protocol.
 ...
 * confidence is the engine own, their source names the strategy that
 ...
 *
 * @param {Array<{word: string, distance: number, confidence: number, source: string}>} dictionary
 * @param {Array<{word: string, score: number}> | null} neighbors
 */
export function mergeSemanticCandidates(dictionary, neighbors) {
