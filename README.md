...
- `lid/` (feature `model`, plan 102): the pure-Rust
  language-identification model reader — the registry's
  `kotoshu://models/lid/lid-176` artifact pair (the fastText lid.176
  conversion, int8-per-row ONNX + vocab sidecar) parsed with no
  runtime, scored bit-faithfully to the fastText bindings
  (sign-extended FNV-1a subword hashing, pruned-ngram mapping,
  hierarchical softmax with the exact sigmoid / `log(x + 1e-5)` math).
  `LidModel::detect(text)` returns `{ code, score }`, matching the gem's
  `LanguageIdentifier` on the frozen parity corpus (labels equal,
  scores within the int8 drift). The wasm surface exposes it as
...
- The ignored integration test (`tests/rerank_integration.rs`) runs
  ...
  `suggest("helo")` reranked with "hello" kept at rank 1. The LID
  sibling (`tests/lid_parity.rs`, `--features model -- --include-ignored`)
  asserts the gem's frozen detection outputs (55-sample multilingual
  corpus, codes exact, scores within 1e-3) over the committed artifact
  pair (`kotoshu/tests/fixtures/lid/`, regenerated by
